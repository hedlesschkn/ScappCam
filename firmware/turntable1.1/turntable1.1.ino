/* TODO
* remove calibration
* move define pins to RAMPS library
* move constant names to SACPP library
* move bitmap to SCAPP library
* additinoal screen size support
*/

//WIRING
//use any of the 5 stepper drivers (E0/1 or x,y,z)
//Hstepper uses Z axis
//no endstops, assumes start is 0
//camera control on servo 1 and 2
...
//reset gear ratio to 1:1 for direct drive turntable
#define gearTeeth 1//540.0 //number of teeth on the device large gear (assuming gear was full in case of arch)
#define stepperTeeth 1//13.0 //number of teeth on the stepper's gear
//number of steps for a full stepper rotation
#define stepper360 6400
//full: 200
//half: 400
//1/4: 800
//1/8: 1600
...
//  u8g.setDefaultForegroundColor();
    ...
    drawScreen();
    delay(5000); //delay for splashscreen
...
void loop() {
//  if (Vstepper.isRunning()){
//    digitalWrite(BRAKE, HIGH); //disengage brake
//  }
//  else{
//    digitalWrite(BRAKE, LOW); //engage brake
//    digitalWrite(X_EN, HIGH); //disabled
//  }

//disable H stepper if not running
  if (Hstepper.isRunning()){
    digitalWrite(Z_EN, LOW); //enable Hstepper
  }
  else{
    digitalWrite(Z_EN, HIGH); //disable Hstepper
  }
...
//                    
//                    while(okReleased != 1){
//                      digitalWrite(X_EN, LOW); //enabled
//                      digitalWrite(BRAKE, HIGH); //disengage brake
//                      readEncoder();
//                      //Vstepper.run();
//                      if (ccw==1) { //move down
//                        if (!digitalRead(X_MIN)){ //check for hitting endstop
...
//                      }
//                    } //while
//                    //digitalWrite(BRAKE, LOW); //engage brake (dealt with in loop instead)
//                    
...
//                    Serial.println("selected set to 0");
                  ...
                  case 1: //Horizontal Angle
                    ...
                    drawScreen();
                    while(okReleased != 1){
                      digitalWrite(Z_EN, LOW); //enable Horizontal stepper
                      readEncoder();
                      Hstepper.run();
                      if (ccw==1) {
                        if (hAngleInt > -360) {
...
void stopscan(){
  if(digitalRead(KILL_PIN) == LOW || estop == HIGH){ //low is pressed, high is released
      ...
      //Vstepper.moveTo(0);
      digitalWrite(BRAKE, LOW); //should already be engaged but double check
    }
}
...
//  myStepper.moveTo(initial_homing+100);//move 1 step up to release brake
//  myStepper.run();
//  delay(500);
...
void setStepperDefaults(){
    ...
    Hstepper.setMaxSpeed(200.0); //200 is scapp default, 
    Hstepper.setAcceleration(100.0); //100 is scapp default
}
...
void whatToDraw() {
    ...
    switch (screen) {
        case 0: { //splash screen
            //menu_max=0;
            u8g.drawStr(32,16,"Turntable");
            u8g.drawStr(32,32,"V1.1");
            u8g.drawStr(32,48,"(motors off)");
